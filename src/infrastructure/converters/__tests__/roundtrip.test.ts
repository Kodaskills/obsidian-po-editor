...
describe('Roundtrip: PO → format → PO', () => {
    it('PO → JSON → PO preserves all entries', () => {
        ...
        expect(finalFile.entries.find(e => e.msgid === 'Fuzzy entry')?.flags).toContain('fuzzy');
        expect(finalFile.entries.find(e => e.msgid === 'OK')?.msgctxt).toBe('Button');
    });
...
describe('Roundtrip: PO → PO (identity)', () => {
    it('parse then compile twice preserves msgid and msgstr', () => {
        ...
        for (let i = 0; i < file1.entries.length; i++) {
            ...
            expect(file1.entries[i].msgstr || file1.entries[i].msgstrPlural?.[0])
                .toEqual(file2.entries[i].msgstr || file2.entries[i].msgstrPlural?.[0]);
        }
    });
});
...
`;
        ...
        expect(compiled).toContain('Multi\\nline');
    });
