...
import { ValidatePOUseCase, formatValidationResult } from '../ValidatePOUseCase';
...
const validateTestCases = [
    ...
    },
    {
        ...
        input: {
            poFile: {
                entries: [
                    { msgid: 'Hello', msgstr: '', flags: [], comments: {}, obsolete: false },
                ],
                obsolete: [],
            },
        },
    ...
    },
    {
        ...
        input: {
            poFile: {
                entries: [
                    { msgid: '', msgstr: 'Something', flags: [], comments: {}, obsolete: false },
                ],
                obsolete: [],
            },
        },
    ...
    },
    {
        ...
        input: {
            poFile: {
                entries: [
                    { msgid: 'one file', msgstr: '', msgidPlural: 'many files', msgstrPlural: [], flags: [], comments: {}, obsolete: false },
                ],
                obsolete: [],
            },
        },
        check: (result: any) => {
            expect(result.errors.some((e: any) => e.message.includes('Plural form defined'))).toBe(true);
        },
    },
    {
        ...
        input: {
            poFile: {
                ...
                obsolete: [
                    { msgid: 'Old', msgstr: 'Vieux', flags: [], comments: {}, obsolete: true },
                ],
            },
        },
        check: (result: any) => {
    ...
    },
    {
        ...
        input: {
            poFile: {
                entries: [
                    { msgid: 'printf', msgstr: '%s', flags: ['c-format', 'no-c-format'], comments: {}, obsolete: false },
                ],
                obsolete: [],
            },
        },
        check: (result: any) => {
            expect(result.warnings.some((w: any) => w.message.includes('Conflicting format flags'))).toBe(true);
        },
    },
    {
        ...
        input: {
            poFile: {
                entries: [
                    { msgid: 'test', msgstr: 'test', flags: ['python-format', 'csharp-format'], comments: {}, obsolete: false },
                ],
                obsolete: [],
            },
        },
        check: (result: any) => {
            expect(result.warnings.some((w: any) => w.message.includes('Multiple format flags'))).toBe(true);
        },
    },
    {
...
describe('formatValidationResult', () => {
    ...
    it('formats errors correctly', () => {
        const result = {
            valid: false,
            errors: [
                { type: 'error', message: 'Test error', entry: 'test' },
            ],
            warnings: [],
    ...
    it('formats warnings correctly', () => {
        const result = {
            ...
            errors: [],
            warnings: [
                { type: 'warning', message: 'Test warning', entry: 'test' },
            ],
            statistics: { total: 1, translated: 0, untranslated: 1, fuzzy: 0 },
        };
