import {
  ...
  ValidatePO,
  type ValidatePOInput,
  type ValidatePOOutput,
...
const validateTestCases = [
  ...
  },
  {
    ...
    check: (result: ValidatePOOutput) => {
      expect(result.valid).toBe(true);
      expect(
        result.warnings.some((w: ValidatePOOutput["warnings"][number]) =>
          w.message.includes("Untranslated"),
        ),
      ).toBe(true);
    },
  },
  ...
  },
  {
    ...
    check: (result: ValidatePOOutput) => {
      ...
      expect(
        result.errors.some((e: ValidatePOOutput["errors"][number]) =>
          e.message.includes("Empty msgid"),
        ),
      ).toBe(true);
    },
  },
  {
    ...
    check: (result: ValidatePOOutput) => {
      ...
      expect(
        result.errors.some((e: ValidatePOOutput["errors"][number]) =>
          e.message.includes("untranslated"),
        ),
      ).toBe(true);
    },
  },
  {
    ...
    check: (result: ValidatePOOutput) => {
      expect(
        result.errors.some((e: ValidatePOOutput["errors"][number]) =>
          e.message.includes("Plural entry"),
        ),
      ).toBe(true);
    },
  },
  ...
  },
  {
    ...
    check: (result: ValidatePOOutput) => {
      expect(
        result.warnings.some((w: ValidatePOOutput["warnings"][number]) =>
          w.message.includes("Conflicting format flags"),
        ),
      ).toBe(true);
    },
  },
  {
    ...
    check: (result: ValidatePOOutput) => {
      expect(
        result.warnings.some((w: ValidatePOOutput["warnings"][number]) =>
          w.message.includes("Multiple format flags"),
        ),
      ).toBe(true);
    },
  },
...
describe("ValidatePO.execute", () => {
  validateTestCases.forEach(({ name, input, check }) => {
    it(name, () => {
      const useCase = new ValidatePO();
      const result = useCase.execute(input as ValidatePOInput);
      check(result);
    });
  });
});
...
describe("formatValidationResult", () => {
  it("formats valid result correctly", () => {
    const result = {
      ...
      errors: [],
      warnings: [],
      statistics: { total: 5, translated: 3, untranslated: 2, fuzzy: 0, obsolete: 0 },
    };
  ...
  it("formats errors correctly", () => {
    const result = {
      ...
      warnings: [],
      statistics: { total: 1, translated: 0, untranslated: 1, fuzzy: 0, obsolete: 0 },
    };
