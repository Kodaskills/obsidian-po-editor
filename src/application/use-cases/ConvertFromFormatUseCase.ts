    ...
    execute(input: ConvertFromFormatUseCaseInput): ConvertFromFormatUseCaseOutput {
        try {
            ...
            if (input.sourceFormat !== 'po') {
                const hasNoTranslations = poFile.entries.every(e => !e.msgstr.trim());
                ...
                if (!input.options?.preserveComments) {
                    const hasComments = poFile.entries.some(e =>
                        e.comments.translator || e.comments.extracted || e.comments.reference
                    );
                    if (hasComments) {
