...
export interface ValidatePOInput {
  poFile: POFile;
  strictMode?: boolean;
}

export interface ValidatePOOutput {
  valid: boolean;
...
export class ValidatePO {
  execute(input: ValidatePOInput): ValidatePOOutput {
    ...
    if (!input.poFile) {
      return {
        ...
        warnings: [],
        statistics: { total: 0, translated: 0, untranslated: 0, fuzzy: 0, obsolete: 0 },
      };
    }

    const stats: ValidatePOOutput["statistics"] = {
...
export function formatValidationResult(result: ValidatePOOutput): string {
  const lines: string[] = [];
