...
const ACTION_COLORS = ['#f39c12', '#e74c3c', '#3498db', '#27ae60', '#9b59b6', '#1abc9c', '#d35400'];
    ...
    display(): void {
        ...
        this.plugin.settings.quickActions.forEach((action: CustomAction, index: number) => {
            const s = new Setting(containerEl)
                ...
                .addExtraButton(btn => btn
                    ...
                    .onClick(async () => {
                        this.plugin.settings.quickActions = this.plugin.settings.quickActions.filter((_: CustomAction, i: number) => i !== index);
                        await this.plugin.saveSettings();
                        this.display();
                    }));
        ...
        new Setting(containerEl)
            .addButton(btn => btn
                ...
                .setCta()
                .onClick(async () => {
                    const nextColor = ACTION_COLORS[this.plugin.settings.quickActions.length % ACTION_COLORS.length];
        ...
        this.plugin.settings.placeholderPatterns.forEach((pattern: string, index: number) => {
            const s = new Setting(containerEl)
                ...
                .addExtraButton(btn => btn
                    ...
                    .onClick(async () => {
                        this.plugin.settings.placeholderPatterns = this.plugin.settings.placeholderPatterns.filter((_: string, i: number) => i !== index);
                        await this.plugin.saveSettings();
                        this.display();
                    }));
            s.infoEl.remove();
