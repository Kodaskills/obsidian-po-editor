...
import { ValidatePOUseCase, formatValidationResult } from '../../application/use-cases/ValidatePOUseCase';
...
export class ValidatePOCommand {
    ...
    async execute(options?: ValidatePOCommandOptions): Promise<{ valid: boolean; result: any } | null> {
        ...
        try {
            ...
            new Notice(`Validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 10000);
            return null;
        }
    }
...
class ValidationResultModal extends require('obsidian').Modal {
...
export function createValidatePOCommand(app: App, plugin: POEditorPlugin): ValidatePOCommand {
    return new ValidatePOCommand(app, plugin);
}