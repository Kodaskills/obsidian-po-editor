import { formatValidationResult, type ParsePO, type ValidatePO } from "@application/index";
import { type POConverter } from "@infrastructure/index";
import { type App, MarkdownView, Modal, Notice, Setting } from "obsidian";
...
export class ValidatePOCommand {
  constructor(
    private app: App,
    private validatePOUseCase: ValidatePO,
  ...
  ): Promise<{ valid: boolean; result: unknown } | null> {
...
class ValidationResultModal extends Modal {
  ...
  onOpen(): void {
    ...
    const heading = new Setting(contentEl)
      .setName(this.isValid ? "✓ Validation Passed" : "✗ Validation Failed")
      .setHeading();
    heading.settingEl.addClass(this.isValid ? "validation-success" : "validation-error");
