...
import { ConvertFromFormatUseCase } from '../../application/use-cases/ConvertFromFormatUseCase';
    ...
    async execute(options: ImportFormatCommandOptions): Promise<TFile | null> {
        ...
        if (options.sourceFile) {
            try {
                ...
                new Notice(`Failed to read file: ${error instanceof Error ? error.message : 'Unknown error'}`, 10000);
                return null;
            }
        } else {
...
class ImportModal extends Modal {
    ...
    onOpen(): void {
        ...
        this.fileInput = fileRow.createEl('input', {
            type: 'text',
