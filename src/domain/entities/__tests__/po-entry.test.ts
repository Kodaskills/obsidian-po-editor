...
import { createPluralEntry, createSingularEntry, isPluralEntry } from "../POEntry";

// Compatibility shims for legacy test expectations
const createPOEntry = (msgid: string, msgstr: string, options?: any) =>
  createSingularEntry(msgid, msgstr, options);
const parsePOEntry = (input: any) => {
  if (input.msgid_plural || (Array.isArray(input.msgstr) && input.msgid_plural)) {
    return createPluralEntry(input.msgid, input.msgid_plural ?? "", input.msgstr ?? [], {
      msgctxt: input.msgctxt,
      flags: input.comments?.flag ? input.comments.flag.split(", ") : [],
...
const serializePOEntry = (input: any) => {
  const result: any = { msgid: input.msgid };
  ...
  if (input.comments && Object.keys(input.comments).length > 0) result.comments = input.comments;
  if (input.flags?.length) result.comments = { ...result.comments, flag: input.flags.join(", ") };
  return result;
};
